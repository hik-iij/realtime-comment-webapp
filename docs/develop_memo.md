...
- 機能案
    ...
    - 投稿はサーバー再起動後に消えるものとする
        - フロントエンドがコメントの履歴(レンダリング前のデータ)をJSON形式でエクスポートできるようにしておく
        - バックエンド側ではエクスポートに関する機能を持たない
    - delete機能
        - 削除は投稿の作成者・ルームの所有者が行える
    - edit機能
        - 編集は投稿の作成者のみが行える
    - ワードミュート機能
    - ユーザごとのフロントエンドの設定をエクスポートする機能
        - JSONあたりで管理
        - ワードミュートやニックネームなど
    - ルーム or コメントルーム管理
        - ランダムなIDが割り当てられる
...
- 返信スレッド
    - リアルタイムひとりごとみたいなものなのでエアリプしてもらえばいい
- ふぁぼ・絵文字リアクション
    - SNSなんだよね2
    - あくまでもリアルタイム性を重視するので
- ユーザのブロック機能・通報機能
...
ルームを削除した場合、メモリ上からもデータを削除し、メモリ領域の解放とデータの復元ができないようにします。
